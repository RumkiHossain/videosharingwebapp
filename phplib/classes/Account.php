...
class Account {
    ...
    public function match($un, $pw) {

        $query = $this->con->prepare("SELECT userType FROM users WHERE username=:un AND password=:pw");
        $query->bindParam(":un", $un);
        $query->bindParam(":pw", $pw);
        ...
        if($query->fetch(PDO::FETCH_ASSOC) == 'user'){
            return true;
        }else{
            return false;
        }
    ...
    public function insertUserDetails($fn, $ln, $un, $em, $pw) {
        ...
        $pw = hash("sha512", $pw);
        $profilePic = "designer/images/profilePicture/profile.png";

        $ut = "user";

        $query = $this->con->prepare("INSERT INTO users (firstName, lastName, username, email, password, profilePic, userType)
                                        VALUES(:fn, :ln, :un, :em, :pw, :pic, :ut)");
